//
//  WrappedResponseTests.swift
//  MeerkatAPI
//
//  Created by Leo Wehrfritz on 31.03.26.
//


import Testing
import Foundation
@testable import MeerkatAPI

@Suite struct WrappedResponseTests {
    @Test func testDecodeContactResponse() throws {
        let json = """
        {
            "contact": {
                "ID": 1,
                "CreatedAt": "1970-01-01T00:00:00Z",
                "UpdatedAt": "1970-01-01T00:00:01Z",
                "DeletedAt": null,
                "firstname": "Matt",
                "lastname": "Smith",
                "nickname": "Matty",
                "gender": "male",
                "email": "matt.smith@example.com",
                "phone": "(555) 1234 567890",
                "birthday": "1982-10-28",
                "photo": "437f3e01-7e7d-4e8a-a3c9-212b33b4229e_photo.jpg",
                "relationships": null,
                "address": "1 Tardis Way",
                "how_we_met": "Watching a TV show",
                "food_preference": "Vegetarian",
                "work_information": "Actor",
                "contact_information": "Somewhere, sometime",
                "circles": [
                    "Doctor Who"
                ],
                "custom_fields": {
                    "Doctor #": "11th Doctor"
                },
                "archived": false
            },
            "message": "Contact created successfully"
        }
        """
        
        let jsonData = Data(json.utf8)
        
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        
        let response = try decoder.decode(WrappedObject<Contact>.self, from: jsonData)
        
        let contact = response.result
        
        #expect(contact.id == 1)
        #expect(contact.createdAt == Date(timeIntervalSince1970: 0))
        #expect(contact.updatedAt == Date(timeIntervalSince1970: 1))
        #expect(contact.deletedAt == nil)
        #expect(contact.firstname == "Matt")
        #expect(contact.lastname == "Smith")
        #expect(contact.nickname == "Matty")
        #expect(contact.gender == .male)
        #expect(contact.email == "matt.smith@example.com")
        #expect(contact.phone == "(555) 1234 567890")
        #expect(contact.birthday == "1982-10-28")
        #expect(contact.photo == "437f3e01-7e7d-4e8a-a3c9-212b33b4229e_photo.jpg")
        #expect(contact.relationships == nil)
        #expect(contact.address == "1 Tardis Way")
        #expect(contact.howWeMet == "Watching a TV show")
        #expect(contact.foodPreference == "Vegetarian")
        #expect(contact.workInformation == "Actor")
        #expect(contact.contactInformation == "Somewhere, sometime")
        #expect(contact.circles == ["Doctor Who"])
        #expect(contact.customFields == ["Doctor #": "11th Doctor"])
        #expect(contact.archived == false)
        #expect(contact.photoThumbnail == nil)
        
        #expect(response.message == "Contact created successfully")
    }
    
    @Test func testDecodeRelationshipResponse() throws {
        let json = """
        {
            "relationship": {
                "ID": 1,
                "CreatedAt": "2026-03-28T23:00:13.0Z",
                "UpdatedAt": "2026-03-28T23:00:13.0Z",
                "DeletedAt": null,
                "name": "David Smith",
                "type": "Father",
                "gender": "",
                "birthday": "",
                "contact_id": 2,
                "related_contact_id": 1,
                "related_contact": {
                    "ID": 1,
                    "CreatedAt": "1970-01-01T00:00:00Z",
                    "UpdatedAt": "1970-01-01T00:00:01Z",
                    "DeletedAt": null,
                    "firstname": "Matt",
                    "lastname": "Smith",
                    "nickname": "Matty",
                    "gender": "male",
                    "email": "matt.smith@example.com",
                    "phone": "(555) 1234 567890",
                    "birthday": "1982-10-28",
                    "photo": "437f3e01-7e7d-4e8a-a3c9-212b33b4229e_photo.jpg",
                    "relationships": null,
                    "address": "1 Tardis Way",
                    "how_we_met": "Watching a TV show",
                    "food_preference": "Vegetarian",
                    "work_information": "Actor",
                    "contact_information": "Somewhere, sometime",
                    "circles": [
                        "Doctor Who"
                    ],
                    "custom_fields": {
                        "Doctor #": "11th Doctor"
                    },
                    "archived": false,
                    "photo_thumbnail": ""
                }
            }
        }
        """
        
        let jsonData = Data(json.utf8)
        
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        
        let response = try decoder.decode(WrappedObject<Relationship>.self, from: jsonData)
        
        let relationship = response.result
        
        #expect(relationship.id == 1)
        #expect(relationship.createdAt.timeIntervalSince1970 == 1774738813)
        #expect(relationship.updatedAt?.timeIntervalSince1970 == 1774738813)
        #expect(relationship.deletedAt == nil)
        #expect(relationship.name == "David Smith")
        #expect(relationship.type == "Father")
        #expect(relationship.gender == .unknown)
        #expect(relationship.birthday == "")
        #expect(relationship.contactId == 2)
        #expect(relationship.relatedContactId == 1)
        
        let contact = relationship.relatedContact
        
        #expect(contact.id == 1)
        #expect(contact.createdAt == Date(timeIntervalSince1970: 0))
        #expect(contact.updatedAt == Date(timeIntervalSince1970: 1))
        #expect(contact.deletedAt == nil)
        #expect(contact.firstname == "Matt")
        #expect(contact.lastname == "Smith")
        #expect(contact.nickname == "Matty")
        #expect(contact.gender == .male)
        #expect(contact.email == "matt.smith@example.com")
        #expect(contact.phone == "(555) 1234 567890")
        #expect(contact.birthday == "1982-10-28")
        #expect(contact.photo == "437f3e01-7e7d-4e8a-a3c9-212b33b4229e_photo.jpg")
        #expect(contact.relationships == nil)
        #expect(contact.address == "1 Tardis Way")
        #expect(contact.howWeMet == "Watching a TV show")
        #expect(contact.foodPreference == "Vegetarian")
        #expect(contact.workInformation == "Actor")
        #expect(contact.contactInformation == "Somewhere, sometime")
        #expect(contact.circles == ["Doctor Who"])
        #expect(contact.customFields == ["Doctor #": "11th Doctor"])
        #expect(contact.archived == false)
        #expect(contact.photoThumbnail == "")
        
        #expect(response.message == nil)
    }
}
